...
def execute_query(session, face, original):
    session.transaction().execute(
        f"insert into faces (face_key, face_name, original_key) values (\"{face}\", null, \"{original}\");",
        commit_tx=True,
    )
...
async def main(request):
    ...
    s3.download_fileobj('itis-2022-2023-vvot44-photos', body['photo'], photo_object)

    img = Image.open(photo_object)
    area = (int(body['coordinates'][0]['x']), int(body['coordinates'][0]['y']), int(body['coordinates'][2]['x']), int(body['coordinates'][2]['y']))
    ...
    in_mem_file.seek(0)
    s3.upload_fileobj(in_mem_file, 'itis-2022-2023-vvot44-faces', f"{filename}.jpg")
